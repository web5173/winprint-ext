...
const DEFAULT_PDFIUM_BUILD_ID: &str = "8009";
...
fn try_link_pdfium() -> Result<(), Box<dyn Error>> {
    ...
    let build_id_raw = env::var("WINPRINT_PDFIUM_BUILD_ID")
        .unwrap_or_else(|_| DEFAULT_PDFIUM_BUILD_ID.to_string());
    // Normalize build id: accept "7891", "chromium/7891" or percent-encoded "chromium%2F7891"
    let build_id = {
        let decoded = build_id_raw.replace("%2F", "/");
        if decoded.contains('/') {
            decoded.split('/').last().unwrap().to_string()
        } else {
            decoded
        }
    };
    ...
    if try_use_local_pdfium(&target_path)? {
        if !verified_marker.exists() {
            fs::write(&verified_marker, "Using local pdfium binaries\n")?;
        }
    }
...
fn try_use_local_pdfium(target_path: &PathBuf) -> Result<bool, Box<dyn Error>> {
    let local_dir = std::env::var("WINPRINT_PDFIUM_LOCAL_DIR").ok();
    let local_dir = match local_dir {
        Some(local_dir) => PathBuf::from(local_dir),
        None => return Ok(false),
    };

    println!("cargo:warning=winprint build script using local pdfium dir: {}", local_dir.display());

    if !local_dir.exists() {
        println!("cargo:warning=local pdfium dir does not exist");
        return Ok(false);
    }
    ...
    for entry in fs::read_dir(&local_dir)? {
        ...
        if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
            ...
            if ["dll", "lib", "so", "a"].contains(&ext.as_str()) {
                let dest = target_path.join(entry.file_name());
                fs::copy(&path, &dest)?;
                found = true;
                println!("cargo:warning=copied local pdfium binary: {}", path.display());
            }
        }
    }

    println!("cargo:warning=local pdfium found={} target={}", found, target_path.display());
